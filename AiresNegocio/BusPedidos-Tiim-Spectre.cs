...
namespace AiresNegocio
{
    public class BusPedidos : BusAbstracta
    {
        ...
        public int AgregaMovimientoTraspaso(int MovimientoId, int AlmacenOrigenId, int AlmacenDestinoId, 
                                            string Comentario, int Estado, int UsuarioId)
        {
            try
            {
                return new DatPedidos().agregaMovimientoTraspaso(MovimientoId, AlmacenOrigenId, AlmacenDestinoId, Comentario, Estado, UsuarioId);
            }
            catch (Exception ex) { throw new Exception(ex.Message); }
        }
