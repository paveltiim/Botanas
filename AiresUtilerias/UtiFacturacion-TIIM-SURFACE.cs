...
namespace AiresUtilerias
{
    public class UtiFacturacion
    {
        ...
        public string Facturar40(EntEmpresa Emisor, EntPedido Pedido, List<EntProducto> ListaProductos, EntCliente Cliente,
                                    string SerieFactura, string FolioFactura, DateTime FechaFactura,
                                    ...
                                    string UsoCFDI, string PathGuardaArchivos)
        {
            ...
            string respuesta = wPade.procesarArchivo(this.Contrato, this.Usuario, this.Contraseña, false, textoCSV, opciones);
            string uuid = "";
        ...
        public string CancelarPADE(EntEmpresa Emisor, string UUID, string Motivo, string UUIDsustitucion, string RfcReceptor, decimal Total)
        {
            ...
            string[] opciones = new string[2] { "PLUGIN_PROCESO:GENERICO_CFDI40:1.0", "CERT_DEFAULT" };
                                    ...
                                    string UsoCFDI, string PathGuardaArchivos)
        {
            ...
            st.WriteLine("observaciones= Por el presente pagaré el suscrito reconozco deber y me obligo a pagar incondicionalmente " +
                "a la orden  de " + Emisor.NombreFiscal + ", la cantidad de $" + Pedido.Total.ToString("0.00") + ". ************* " + Cliente.Nombre + " ________________________________");
                            ...
                            string UsoCFDI, string PathGuardaArchivos)
        {
            ...
            st.WriteLine("tipoDocumento=FACTURA");
            st.WriteLine("observaciones= Por el presente pagaré el suscrito reconozco deber y me obligo a pagar incondicionalmente " +
                "a la orden  de " + Emisor.NombreFiscal + ", la cantidad de $" + importeTotal.ToString("0.00") + ". ************* " + Cliente.Nombre + " ________________________________");
            //+ " Facturación Versión 3.3 - TIIM Tecnología | www.tiimtecnologia.com |"); //Facturación Versión 3.3 - TIIM Tecnología | www.tiimtecnologia.com |");
            st.WriteLine("platillaPDF=custom");//gti_clasica");// clasic");// custom");
            st.WriteLine("colorLetraE=000000");
            st.WriteLine("colorPlantillaHex=000000");
                               ...
                               string TipoRelacion, string UUIDRelacionado)
        {
            ...
            st.WriteLine("colorLetraE=000000");
            st.WriteLine("colorPlantillaHex=000000");
            st.WriteLine("logotipo="+Emisor.Logo); //lg_de198d9f5c5960f7ac72e6f - Serdan //lg_a0f8b790af59b2e36be4cf1//TIIM 
            ...
            byte[] byteXML = System.Convert.FromBase64String(r_wsconect.xmlBase64);
            System.IO.FileStream swxml = new System.IO.FileStream((PathGuardaArchivos + ("\\" + (Serie + FolioFactura + ".xml"))), System.IO.FileMode.Create);
            swxml.Write(byteXML, 0, byteXML.Length);
            swxml.Close();
            if (reqt.generarCBB)
            {
                byte[] byteCBB = System.Convert.FromBase64String(r_wsconect.cbbBase64);
                System.IO.FileStream swcbb = new System.IO.FileStream((PathGuardaArchivos + ("\\" + (Serie + FolioFactura + ".png"))), System.IO.FileMode.Create);
                swcbb.Write(byteCBB, 0, byteCBB.Length);
                swcbb.Close();
            }
            if (reqt.generarPDF)
            {
                byte[] bytePDF = System.Convert.FromBase64String(r_wsconect.pdfBase64);
                System.IO.FileStream swpdf = new System.IO.FileStream((PathGuardaArchivos + ("\\" + (Serie + FolioFactura + ".pdf"))), System.IO.FileMode.Create);
                swpdf.Write(bytePDF, 0, bytePDF.Length);
                swpdf.Close();
            }
            if (reqt.generarTXT)
            {
                byte[] byteTXT = System.Convert.FromBase64String(r_wsconect.txtBase64);
                System.IO.FileStream swtxt = new System.IO.FileStream((PathGuardaArchivos + ("\\" + (Serie + FolioFactura + ".txt"))), System.IO.FileMode.Create);
                swtxt.Write(byteTXT, 0, byteTXT.Length);
                swtxt.Close();
            }
                                                ...
                                                string PathGuardaArchivos)
        {
            ...
            st.WriteLine("logotipo="+Emisor.Logo);//lg_de198d9f5c5960f7ac72e6f//SERDAN//lg_a0f8b790af59b2e36be4cf1//TIIM //+ Emisor.Logo
            ...
            byte[] byteXML = System.Convert.FromBase64String(r_wsconect.xmlBase64);
            System.IO.FileStream swxml = new System.IO.FileStream((PathGuardaArchivos + ("\\" + ("CP" + FolioComplemento + ".xml"))), System.IO.FileMode.Create);
            swxml.Write(byteXML, 0, byteXML.Length);
            swxml.Close();
            if (reqt.generarCBB)
            {
                byte[] byteCBB = System.Convert.FromBase64String(r_wsconect.cbbBase64);
                System.IO.FileStream swcbb = new System.IO.FileStream((PathGuardaArchivos + ("\\" + ("CP" + FolioComplemento + ".png"))), System.IO.FileMode.Create);
                swcbb.Write(byteCBB, 0, byteCBB.Length);
                swcbb.Close();
            }
            if (reqt.generarPDF)
            {
                byte[] bytePDF = System.Convert.FromBase64String(r_wsconect.pdfBase64);
                System.IO.FileStream swpdf = new System.IO.FileStream((PathGuardaArchivos + ("\\" + ("CP" + FolioComplemento + ".pdf"))), System.IO.FileMode.Create);
                swpdf.Write(bytePDF, 0, bytePDF.Length);
                swpdf.Close();
            }
            if (reqt.generarTXT)
            {
                byte[] byteTXT = System.Convert.FromBase64String(r_wsconect.txtBase64);
                System.IO.FileStream swtxt = new System.IO.FileStream((PathGuardaArchivos + ("\\" + ("CP" + FolioComplemento + ".txt"))), System.IO.FileMode.Create);
                swtxt.Write(byteTXT, 0, byteTXT.Length);
                swtxt.Close();
            }
                               ...
                               string Observaciones)
        {
            ...
            decimal subtotal = Math.Round(Math.Round(Total, 2) / (1 + IEPS),2); //Math.Round(total / (1 + IVA), 2);
            ...
            if (incluyeIeps)
            {
                ...
                cantidadIeps = (Math.Round(Total, 2) * (IEPS * 100)) / porcentaje;
                importe = Math.Round(Total, 2) - cantidadIeps;
            }
            else
            {
                //ivas = 0.00m;
                importe = Math.Round(Math.Round(Total, 2), 4);
                //incluyeSinIVA = true;
            }
            ...
            if (cantidadIeps > 0)//SI LLEVA CON IEPS SE PONEN LOS DOS SINO SOLO IVA
            {
                ...
                st.WriteLine("Impuestos.Traslados.Impuesto =[002,003]");//003 IEPS
                st.WriteLine("Impuestos.Traslados.TipoFactor =[" + Emisor.TipoFactor + ",Tasa]");//Tasa|Cuota|Exento
                st.WriteLine("Impuestos.Traslados.TasaOCuota =[" + IVA.ToString().PadRight(8, '0') + ","
                                                                 + IEPS.ToString().PadRight(8, '0') + "]");// 0.160000
                st.WriteLine("Impuestos.Traslados.Importe =[" + IVA.ToString().PadRight(8, '0') + ","
                                                              + cantidadIeps.ToString("0.0000") + "]");//ESTO DEBE SER CANTIDADIEPS
            }
            else
            {
                st.WriteLine("Impuestos.Traslados.Base = [" + importe.ToString("0.0000") + "]");//p.PrecioSinIVA.ToString("0.00") + "]");
                st.WriteLine("Impuestos.Traslados.Impuesto =[002]");//002
                st.WriteLine("Impuestos.Traslados.TipoFactor =[" + Emisor.TipoFactor.Replace("\r", "").Replace("\n", "") + "]");//Tasa|Cuota|Exento
                st.WriteLine("Impuestos.Traslados.TasaOCuota =[" + 0.16m.ToString().PadRight(8, '0') + "]");// 0.160000
                if (Emisor.TipoFactorId > 1)// CUOTA|EXENTO|
                    st.WriteLine("Impuestos.Traslados.Importe =[0.0000]");
                else
                    st.WriteLine("Impuestos.Traslados.Importe =[" + cantidadIeps.ToString("0.0000") + "]");
            }
            ...
            //{
            //TRASLADOS TOTALES
            if (CantidadIVA > 0)//SI LLEVA CON IEPS SE PONEN LOS DOS SINO SOLO IVA
            {
                st.WriteLine("[Traslados]");
                //st.WriteLine("TotalImpuestosTrasladados=" + (CantidadIVA + CantidadIEPS).ToString("0.00"));
                st.WriteLine("TotalImpuestosTrasladados=" + (CantidadIVA).ToString("0.00"));
                st.WriteLine("Impuesto=[002,003]");
                    ...
                    st.WriteLine("TasaOCuota=[" + IVA.ToString().PadRight(8, '0') + "," + IEPS.ToString().PadRight(8, '0') + "]");
                //st.WriteLine("Importe=[" + CantidadIVA.ToString("0.00") + "," + CantidadIEPS.ToString("0.00") + "]");
                st.WriteLine("Importe=[" + 0.ToString("0.00") + "," + CantidadIVA.ToString("0.00") + "]");
            }
                else
                {
                    st.WriteLine("[Traslados]");
            ...
            byte[] byteXML = System.Convert.FromBase64String(r_wsconect.xmlBase64);
            System.IO.FileStream swxml = new System.IO.FileStream((PathGuardaArchivos + ("\\" + (Serie+ FolioNotaCredito + ".xml"))), System.IO.FileMode.Create);
            swxml.Write(byteXML, 0, byteXML.Length);
            swxml.Close();
            if (reqt.generarCBB)
            {
                byte[] byteCBB = System.Convert.FromBase64String(r_wsconect.cbbBase64);
                System.IO.FileStream swcbb = new System.IO.FileStream((PathGuardaArchivos + ("\\" + (Serie + FolioNotaCredito + ".png"))), System.IO.FileMode.Create);
                swcbb.Write(byteCBB, 0, byteCBB.Length);
                swcbb.Close();
            }
            if (reqt.generarPDF)
            {
                byte[] bytePDF = System.Convert.FromBase64String(r_wsconect.pdfBase64);
                System.IO.FileStream swpdf = new System.IO.FileStream((PathGuardaArchivos + ("\\" + (Serie + FolioNotaCredito + ".pdf"))), System.IO.FileMode.Create);
                swpdf.Write(bytePDF, 0, bytePDF.Length);
                swpdf.Close();
            }
            if (reqt.generarTXT)
            {
                byte[] byteTXT = System.Convert.FromBase64String(r_wsconect.txtBase64);
                System.IO.FileStream swtxt = new System.IO.FileStream((PathGuardaArchivos + ("\\" + (Serie + FolioNotaCredito + ".txt"))), System.IO.FileMode.Create);
                swtxt.Write(byteTXT, 0, byteTXT.Length);
                swtxt.Close();
            }
        ...
        public string Cancelar(EntEmpresa Emisor, string UUID, string Motivo, string FolioSustitucion)
        {
            ...
            string url = Emisor.LinkCancelacionFactura; // "http://44.234.130.5.ddns.net/alka/inicioApp/CancelarFacPro";
            data["RfcEmisor"] = Emisor.RFC;// "ESI920427886";
            data["UUID"] = UUID; // "ABC1147C-D41E-4596-9C3E-45629B097CDB";
            data["Motivo"] = Motivo;//"02"; 
            data["UUIDRelacion"] = FolioSustitucion;
