    ...
    public partial class SalidasInsumo : FormBase
    {
        ...
        private void btnRefrescarEntradas_Click(object sender, EventArgs e)
        {
            try
            {
                base.SetWaitCursor();
                EntCatalogoGenerico almacen = ObtieneCatalogoGenericoFromCmb(cmbAlmacenes);
                if (rdoSalidasPorMes.Checked)
                    CargaSalidas(dtpFechaSalidasPorDia.Value.Date, dtpFechaSalidasPorDia.Value.Date.AddDays(1), almacen.Id);
                else if (rdoSalidasPorMes.Checked)
                {
                    if (cmbMesesEntradas.SelectedIndex >= 0)
                        CargaSalidas(FechaDesdeFromComboBoxs(cmbMesesEntradas, cmbAñoEntradas),
                                        FechaHastaFromComboBoxs(cmbMesesEntradas, cmbAñoEntradas).AddDays(1),
        ...
        private void btnExportaSalidasPorMes_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                ImpresionSalidasDetalle vImprimeSalidas = new ImpresionSalidasDetalle(listaProductos);
                vImprimeSalidas.Show();
        ...
        private void rdoEntradasPorMes_CheckedChanged(object sender, EventArgs e)
        {
            try
            {
                pnlEntradasPorMes.Enabled = rdoSalidasPorMes.Checked;
                pnlPorDiaEntradas.Enabled = !rdoSalidasPorMes.Checked;
                btnRefrescarSalidasInsumos.PerformClick();
            }
            catch (Exception ex) { MuestraExcepcion(ex); }
        }

        private void btnImprimeSalidasPorDia_Click(object sender, EventArgs e)
        {
            try
            {
                base.SetWaitCursor();
