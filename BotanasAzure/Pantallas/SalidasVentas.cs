...
{
    public partial class SalidasVentas : FormBase
    {   
        void AgregaProductoEnPedido(EntProducto ProductoSeleccionado, decimal CantidadAgrega)
        {
            ...
            if (CantidadAgrega > 0)
            {
                ...
                //SIEMPRE BUSCA EL PRECIO MENUDEO INICIALMENTE
                decimal precioPorCantidad = new BusProductos().ObtienePrecioProductoCantidad(0, ProductoSeleccionado.Id, CantidadAgrega).PrecioVenta;

                if (precioPorCantidad > 0)
                {
                    //INCREMENTAR EL 2% en PUNTO DE VENTA
                    precioPorCantidad = Math.Round(precioPorCantidad * 1.02m, 2);
                ...
                //PROBAR COMO LO CALCULABA PAGO CON TARJETA
                //if (cmbFormaPago.Text.Contains("04") || cmbFormaPago.Text.Contains("28")) //PAGO CON TARJETA
                //    ProductoSeleccionado.PrecioVenta = Math.Round((ProductoSeleccionado.PrecioVentaSinIVA + ProductoSeleccionado.IEPS) * 1.03m, 2);
                //else
        ...
        List<EntProducto> ListaProductosMinMax = new List<EntProducto>();
...
#region Metodos
        ...
        public void CargaProductos(int AlmacenId, int TipoPedidoId)
        {
            this.ListaProductos = new BusProductos().ObtieneProductosExistenciaPorAlmacen(Program.UsuarioSeleccionado.CompañiaId, 0, AlmacenId, TipoPedidoId)
                                                                                        .OrderBy(P => P.Descripcion).ToList();
            gvProductosBusqueda.DataSource = this.ListaProductos;
        }
        public void CargaProductosExistenciaMinMax(int AlmacenId)
        {
            this.ListaProductosMinMax = new BusProductos().ObtieneProductosExistenciaMinMax(Program.UsuarioSeleccionado.CompañiaId, AlmacenId)
                                                                                        .OrderBy(P => P.Descripcion).ToList();
            gvProductosExistenciasMinMax.DataSource = this.ListaProductosMinMax;
            gvProductosExistenciasMinMax.Refresh();
        }
        ...
        /// </summary>
        /// <param name="pedido"></param>
        public void AgregarProductoDetallePedidoBD(int PedidoId, List<EntProducto> ProductosSeleccionados)
        {
        ...
        /// <returns></returns>
        public int AgregarPagoBD(int PedidoId, decimal Cantidad, int FormaPagoId, string FormaPago)
        {
        ...
        /// <returns></returns>
        public int AgregarPagoPedidoBD(int AlmacenCajaId, int PedidoId, decimal Cantidad, int FormaPagoId, string FormaPago, DateTime FechaPago)
        {
            //SeleccionaEstablecimiento vEstablecimiento = new SeleccionaEstablecimiento(AlmacenCajaId,"Depositar en");
            //vEstablecimiento.ShowDialog();
            ...
            return new BusPedidos().AgregaPagoPedido(AlmacenCajaId, pago);
        }
        ...
        void VerificaExistenciaProductosSeleccionados(List<EntProducto> ProductosSeleccionados)
        {
            foreach (EntProducto p in ProductosSeleccionados)
            {
                if (p.Cantidad > p.Existencia)
                    MandaExcepcion("EL PRODUCTO " + p.Codigo + "-" + p.Descripcion + " NO TIENE EXISTENCIA SUFICIENTE");
            }
        }
        void VerificaExistenciaProducto(EntProducto p)
        {
                if (p.Cantidad > p.Existencia)
                    MandaExcepcion("EL PRODUCTO " + p.Codigo + "-" + p.Descripcion + " NO TIENE EXISTENCIA SUFICIENTE");         
        }
        ...
        private void cmbAlmacenes_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                base.SetWaitCursor();
                gvProductosPedido.DataSource = null;
                gvProductosExistenciasMinMax.DataSource = null;
...
#region EVENTOS GV
        ...
        private void gvProductosPedido_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                if ((e.ColumnIndex == cantidadDataGridViewTextBoxColumn.Index 
                    || e.ColumnIndex == PrecioVentaSinIVA.Index 
                    || e.ColumnIndex == Precio.Index) && e.RowIndex > -1)//CANTIDAD | PRECIO SIN IVA | PRECIO
                {
                    ...
                    if (e.ColumnIndex == cantidadDataGridViewTextBoxColumn.Index)//CANTIDAD 
                    {
                        ...
                        {
                            //	OPCION 1:
                            //  CLIENTE TIPO MAYOREO O MENUDEO + CANTIDAD PRODUCTO NECESARIA PARA MAYOREO = PRECIO MAYOREO
                            ...
                            precioPorCantidad = new BusProductos().ObtienePrecioProductoCantidad(0, productoSeleccionado.Id, productoSeleccionado.Cantidad).PrecioVenta;
                            if (precioPorCantidad > 0)
                            {
                                //INCREMENTAR EL 2% en PUNTO DE VENTA
                                precioPorCantidad = Math.Round(precioPorCantidad * 1.02m, 2);
                                ...
                                //PROBAR COMO FUNCIONABA CON PAGO CON TARJETA
                                this.ProductoSeleccionado.PrecioVenta = this.ProductoSeleccionado.PrecioVentaSinIVA + this.ProductoSeleccionado.IEPS;
                            }
                            //}
                            ////}
                    ...
                    else if (e.ColumnIndex == PrecioVentaSinIVA.Index) //PRECIO SIN IEPS
                    {
                        if (productoSeleccionado.IncluyeIeps)
                        {
                    ...
                    else if (e.ColumnIndex == Precio.Index) //PRECIO 
                    {
                        if (productoSeleccionado.IncluyeIeps)
                        {
        ...
        string CreaPathClienteDirectorioFacturas(string PathFacturaBase, string NombreCliente, string NumeroFactura) {
                                ...
                                string UsoCFDI, bool Timbrar)
        {
            ...
            Cliente.NombreFiscal = txtNombreFiscal.Text.Trim();
            ...
            if (Program.ConexionIdActual == 1 && Timbrar && Program.EmpresaSeleccionada.Facturacion)//PRODUCCION
            {
                ...
                this.PathClienteDirectorioFacturas = CreaPathClienteDirectorioFacturas(pathFacturasBase, Cliente.Nombre, Pedido.Factura);
                        ...
                        string Observaciones, string TipoComprobante, bool Timbrar, bool PreFactura)
        {
            ...
            try
            {
                ...
                factura = TimbrarFactura(PedidoAgrega, ProductosSeleccionados, this.ClienteSeleccionado, DateTime.Now,
                ...
                if (!PreFactura)
                {
                    ...
                    try
                    {
                        ...
                        base.EnviaCorreo("FACTURA", Program.EmpresaSeleccionada, PedidoAgrega, this.ClienteSeleccionado, 
                                         this.PathClienteDirectorioFacturas, "","", factura.UUID);
                    }
                    catch (Exception ex)
                    {
        ...
        private void btnAgregar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (MuestraMensajeYesNo(mensaje, "CONFIRMAR") == DialogResult.Yes)
                {
                    ...
                    VerificaExistenciaProductosSeleccionados(this.ListaProductosPedido);
                    ...
                    try
                    {
                        AgregarProductoDetallePedidoBD(this.PedidoAgrega.Id, this.ListaProductosPedido);
                    ...
                    if (chkPagada.Checked)
                    {
                        try
                        {
                            pagoId = AgregarPagoBD(this.PedidoAgrega.Id, pago, ConvierteTextoAInteger(cmbFormaPago.Text.Remove(2)), cmbFormaPago.Text.Remove(0, 4));
                            ...
                            //REVISAR PORQUE EL PAGO SE PASA COMO POSITIVO. REVISAR EN BD PRUEBAS.
                        }
                    }
...
#region REGISTROS
        ...
        public void CargaPedidos(int EstablecimientoId, DateTime FechaDesde, DateTime FechaHasta)
        {
            ...
            gvPedidos.DataSource = this.ListaPedidos;//ESTE FILTRADO ES DIOQUIS, SIEMPRE PASA POR FILTRO.OrderByDescending(P => P.FacturaId).ToList();
        ...
        private void chkFacturar_CheckedChanged(object sender, EventArgs e)
        {
            try {
                ...
                //cmbFormaPagoCopia.Visible = !chkFacturar.Checked;
        ...
        private void cmbFormaPago_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                cmbFormaPagoCopia.SelectedIndex = cmbFormaPago.SelectedIndex;
                //if (gvProductosPedido.Rows.Count > 0)
                //{
                //    //PROBAR COMO LO CALCULABA PAGO CON TARJETA
                //    if (cmbFormaPago.Text.Contains("04") || cmbFormaPago.Text.Contains("28"))//PAGO CON TARJETA AUMENTA
                //    {
                //        foreach (EntProducto p in this.ListaProductosPedido)
                //        {
                        ...
                        try
                        {
                            ...
                            if (facturasEncontradas.Count > 0)
                            {
                                VerificaExistenArchivosFactura(facturasEncontradas[0].Ruta,
                                                                ...
                                                                facturasEncontradas[0].PDF,
                                                                facturasEncontradas[0].XML);
                                uuid = facturasEncontradas[0].UUID;
                ...
                if (vSelFac.ShowDialog() == DialogResult.OK)
                {
                    ...
                    if (vComplePago.ShowDialog() == DialogResult.OK)
                    {
                        ...
                        foreach (EntPedido p in pedidosfacturasSeleccionadas)
                        {
                            ...
                            try
                            {
                                pagoId = AgregarPagoPedidoBD(this.AlmacenId, p.Id, p.PagoTotal, vComplePago.FormaPagoId, vComplePago.FormaPago, vComplePago.FechaPago);
                            }
                            catch (Exception ex)
                            {
        ...
        private void gvProductosPedido_KeyPress(object sender, KeyPressEventArgs e)
        {
            try
            {
                if (gvProductosPedido.CurrentCell != null)
                {
                    ...
                    if (e.KeyChar == (char)Keys.Enter && gvProductosPedido.CurrentCell.ColumnIndex > 0)
                    {
                        txtBuscaProductoCodigo.Focus();
                    }
                }
        ...
        private void btnImprimeNV_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                ImprimirNotaVenta(tituloImpresion, this.ClienteSeleccionado, 
                                this.PedidoAgrega, new BusProductos().ObtieneProductosPorPedido(this.PedidoAgrega.Id),
        ...
        private void cmbFormaPagoCopia_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                cmbFormaPago.SelectedIndex = cmbFormaPagoCopia.SelectedIndex;
            }
            catch (Exception ex)
            {
                MuestraExcepcion(ex);
        ...
        void FiltraProductosMinMax()
        {
            List<EntProducto> lstFiltro = this.ListaProductosMinMax.Where(c => c.Descripcion.ToUpper().Contains(txtBuscaProductoExistencias.Text.ToUpper())
                                                                            || c.Codigo.ToUpper().Contains(txtBuscaProductoExistencias.Text.ToUpper())).ToList();
                            //from c in this.ListaProductosMinMax
                            //where c.Descripcion.ToUpper().Contains(txtBuscaProductoExistencias.Text.ToUpper())
                            //   || c.Codigo.ToUpper().Contains(txtBuscaProductoExistencias.Text.ToUpper())
                            //select c;

            if (!chkExistenciaMin.Checked)
                lstFiltro.RemoveAll(P => P.Existencia < P.ExistenciaMinima);
            if (!chkExistenciaMax.Checked)
                lstFiltro.RemoveAll(P => P.Existencia > P.ExistenciaMaxima);
            if (!chkExistenciaMenorMax.Checked)
                lstFiltro.RemoveAll(P => P.Existencia < P.ExistenciaMaxima );
            if (!chkSinExistencia.Checked)
                lstFiltro.RemoveAll(P => P.Existencia == 0);
            if (!chkSinPedido.Checked)
                lstFiltro.RemoveAll(P => P.Cantidad == 0);
            //lstFiltro.RemoveAll(this.ListaProductosMinMax.Where(P => P.Existencia == 0).ToList());
            gvProductosExistenciasMinMax.DataSource = lstFiltro;
            gvProductosExistenciasMinMax.Refresh();
        }

        private void btnRefrescaExistenciasMinMax_Click(object sender, EventArgs e)
        {
            try
            {
                base.SetWaitCursor();
                ...
                CargaProductosExistenciaMinMax(establecimiento.Id);
                FiltraProductosMinMax();
                //btnFiltrarPedidos.PerformClick();
            }
            catch (Exception ex) { MuestraExcepcion(ex); }
            finally { base.SetDefaultCursor(); }
        ...
        private void chkExistenciaMin_CheckedChanged(object sender, EventArgs e)
        {
            try
            {
                base.SetWaitCursor();
        ...
        private void gvPedidos_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {

        }

        private void chkVerFacturasCanceladas_CheckedChanged(object sender, EventArgs e)
        {
