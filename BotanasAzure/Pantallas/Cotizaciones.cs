...
{
    public partial class Cotizaciones : FormBase
    {
...
#region Metodos
        ...
        public EntPedido AsignaDatosPedido(int EstablecimientoId, int TipoPedidoId, int ClienteId,
        ...
        private void Ventas_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                CargaClientes();
                //}
                tcGeneral.TabPages.Remove(tpRegistrosCotizaciones);
            }
            catch (Exception ex)
            {
                MuestraExcepcion(ex);
...
#region EVENTOS GV
        ...
        private void btnAgregar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                int tipoMovimientoId = (int)TipoMovimiento.COTIZACION;
                int tipoPedidoId = (int)TipoPedido.COTIZACION;
                string numFactura = "";
                string tipoMovimiento = "-Cotización-";
                ...
                if (MuestraMensajeYesNo(mensaje, "CONFIRMAR") == DialogResult.Yes)
                {
                    ...
                    this.PedidoAgrega = AsignaDatosPedido(this.AlmacenId, tipoPedidoId, this.ClienteSeleccionado.Id,
                    ...
                    //if (!facturado)
                    //{
                    //string tituloImpresion = " - COTIZACIÓN - ";
                    ...
                    pedidoImprime.NumOrden = "COT-" + new Random().Next(0,99999).ToString().PadLeft(6, '0');
                    pedidoImprime.SubTotal = ConvierteTextoADecimal(txtSubtotal);
                    pedidoImprime.IVA = 0;
                    pedidoImprime.IEPS = ConvierteTextoADecimal(txtIEPS);
                    pedidoImprime.Total = ConvierteTextoADecimal(txtTotal);
                    //pedidoImprime.TotalUSD = pedidoImprime.Total;
                    pedidoImprime.Observaciones = "";
                    ImprimirSalida(this.ClienteSeleccionado, pedidoImprime, this.ListaProductosPedido,
