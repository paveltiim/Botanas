...
namespace Aires.Pantallas
{
    public partial class MuestraPagos : FormBase
    {
        ...
        private void btnCancelar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (MuestraMensajeYesNo(string.Format("¿Seguro desea ELIMINAR el pago seleccionado? \n Fecha Pago:{0} \n Cantidad Pago:{1}", pagoSeleccionado.FechaPago.ToShortDateString(), FormatoMoney(pagoSeleccionado.Cantidad)), "CONFIRMACIÓN") == DialogResult.Yes)
                {
                    ...
                    new SalidasVentas().EliminaPago(pagoSeleccionado.PedidoId, pagoSeleccionado.Id,pagoSeleccionado.Cantidad*-1         );
        ...
        private void gvPagos_ColumnHeaderMouseClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            try
            {
                if (e.ColumnIndex == fechaPagoDataGridViewTextBoxColumn.Index)
                {
                ...
                else if (e.ColumnIndex == fechaDataGridViewTextBoxColumn.Index)
                {
                    if (((DataGridView)sender).Columns[e.ColumnIndex].HeaderCell.SortGlyphDirection == SortOrder.None || ((DataGridView)sender).Columns[e.ColumnIndex].HeaderCell.SortGlyphDirection == SortOrder.Descending)
                    {
                ...
                else if (e.ColumnIndex == FacturaColumnGvPagos.Index)
                {
                    if (((DataGridView)sender).Columns[e.ColumnIndex].HeaderCell.SortGlyphDirection == SortOrder.None || ((DataGridView)sender).Columns[e.ColumnIndex].HeaderCell.SortGlyphDirection == SortOrder.Descending)
                    {
                ...
                else if (e.ColumnIndex == DescripcionColumnGvPagos.Index)
                {
                    if (((DataGridView)sender).Columns[e.ColumnIndex].HeaderCell.SortGlyphDirection == SortOrder.None || ((DataGridView)sender).Columns[e.ColumnIndex].HeaderCell.SortGlyphDirection == SortOrder.Descending)
                    {
        ...
        {
            try {
                gvPagos.DataSource = this.ListaPagos.Where(P => P.Factura.Contains(txtFactura.Text)).ToList();
                gvPagos.Refresh();
            }
            catch (Exception ex) { MessageBox.Show(ex.Message, "ERROR", MessageBoxButtons.OK, MessageBoxIcon.Error); }
        }
        ...
        private void btnExporta_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                List<EntPago> listaPagos = ObtieneListaPagosFromGV(gvPagos);

                ImpresionPagosCliente vImprimePagos = new ImpresionPagosCliente(listaPagos);
                vImprimePagos.Show();
            }
            catch (Exception ex) { MuestraExcepcion(ex); }
