...
{
    public partial class EstadoDeCuentaFacturasVencidas : FormBase
    {
        public EstadoDeCuentaFacturasVencidas(int EstablecimientoId)
        {
            InitializeComponent();
        ...
        {
            this.ListaPedidos = new BusPedidos().ObtienePedidosClientesDeudaDiasCredito(this.EstablecimientoId);
            ...
            ReportParameter parm2 = new ReportParameter("Cliente", "TODOS");
            ...
            rvPedidosDeudaPorCliente.LocalReport.DataSources.Clear();
            rvPedidosDeudaPorCliente.LocalReport.DataSources.Add(new Microsoft.Reporting.WinForms.ReportDataSource("dsPedidosClientesDeuda", this.ListaPedidos));
            rvPedidosDeudaPorCliente.RefreshReport();
        }
        public void FiltraPedidosClientesCredito(List<EntPedido> ListaPedidos, int DiasVencimiento)
        {

            List<EntPedido> pedidosFiltrados = ListaPedidos
                .Where(p => p.DiasVencidos >= DiasVencimiento)
                .ToList();

            EntPedidoBindingSource.DataSource = pedidosFiltrados;

            rvPedidosDeudaPorCliente.LocalReport.DataSources.Clear();
            rvPedidosDeudaPorCliente.LocalReport.DataSources.Add(new Microsoft.Reporting.WinForms.ReportDataSource("dsPedidosClientesDeuda", pedidosFiltrados));
            rvPedidosDeudaPorCliente.RefreshReport();

            MuestraTotales(pedidosFiltrados.Sum(p => p.Debe));
        }

        void MuestraTotales(decimal SumaFacturasVencimiento)
        {
            //decimal totalFacturasVencidas = this.ListaPedidos.Sum(P => P.Debe);
            // Suma total de la lista base
            decimal totalFacturasVencidas = this.ListaPedidos.Sum(p => p.Debe);

            // Suma total de la lista filtrada
            decimal sumaFiltrada = SumaFacturasVencimiento;
            ...
            {
                porcentaje = (sumaFiltrada / totalFacturasVencidas) * 100;
            }

            txtTotalFacturasVencidas.Text = FormatoMoney(totalFacturasVencidas);
            txtFacturasVencimiento.Text = FormatoMoney(sumaFiltrada);
            txtPorcentajeVencimiento.Text = porcentaje.ToString("N2")+"%";
        }
        ...
        private void EstadoDeCuentaFacturasVencidas_Load(object sender, EventArgs e)
        {
            try
            {
                CargaGvPedidosClientesCredito();
                MuestraTotales(this.ListaPedidos.Sum(p => p.Debe));
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message);
        ...
        private void txtFiltroDiasVencidos_TextChanged(object sender, EventArgs e)
        {
            int diasVencimiento =ConvierteTextoAInteger(txtFiltroDiasVencidos);
            FiltraPedidosClientesCredito(this.ListaPedidos, diasVencimiento);
            lbVencimientoMayorA.Text = "Facturas Vencimiento Mayor a "+diasVencimiento.ToString();
        }
