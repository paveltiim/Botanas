...
namespace Aires.Pantallas
{
    public partial class Entradas : FormBase
    {
        ...
        {
            this.ListaProductos = new BusProductos().ObtieneProductosPorAlmacen(Program.UsuarioSeleccionado.CompañiaId, 0, almacenId).OrderBy(P => P.Descripcion).ToList();
            ...
            try
            {
                ...
                if (MuestraMensajeYesNo(mensaje, "CONFIRMAR") == DialogResult.Yes)
                {
                    ...
                    int movimientoId = new BusPedidos().AgregaMovimientoMaster(txtDescripcionEntrada.Text,
                                                                                ...
                                                                                almacenId, 0, Program.UsuarioSeleccionado.Id);
        ...
        private void btnExportaEntradasPorMes_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (rdoPorDiaEntradas.Checked)
                    dia = " - "+dtpFechaEntradasFiltro.Value.Day.ToString().PadLeft(2,'0')+"\n"+ culturaEspanol.DateTimeFormat.GetDayName(dtpFechaEntradasFiltro.Value.DayOfWeek);
                foreach (EntCatalogoGenerico en in listaEntradasSeleccionadas)
                {
                    List<EntProducto> listaProductosIngreso = new BusProductos().ObtieneMovimientosDetalleProductosSinDatosFactura(1, en.Id, en.IdSecundario);//MOVIMIENTOID | INGRESOID
                    if (en.IdSecundario == 0)
        ...
        private void chkCambiaFechaHora_CheckedChanged(object sender, EventArgs e)
        {
            dtpFechaEntrada.Enabled = chkCambiaFechaHora.Checked;
            dateTimePicker1.Enabled = chkCambiaFechaHora.Checked;
        }

        private void dateTimePicker1_ValueChanged(object sender, EventArgs e)
        {

            dtpFechaEntrada.Value = dateTimePicker1.Value;
        }

        private void dtpFechaEntrada_ValueChanged(object sender, EventArgs e)
        {
            dateTimePicker1.Value = dtpFechaEntrada.Value;
        }
    }
}
