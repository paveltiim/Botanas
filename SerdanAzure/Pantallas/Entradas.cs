...
namespace Aires.Pantallas
{
    public partial class Entradas : FormBase
    {
        ...
        {
            this.ListaProductos = new BusProductos().ObtieneProductosPorAlmacen(0,almacenId);//.OrderBy(P => P.Descripcion).ToList();
        ...
        private void btnAgregar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (MuestraMensajeYesNo(mensaje, "CONFIRMAR") == DialogResult.Yes)
                {
                    ...
                    //pedidoAgrega = AgregarPedido(ClienteSeleccionado.Id, detallePedido.Remove(detallePedido.Length - 2), "", "", ConvierteTextoADecimal(txtTotal.Text), 0, DateTime.Now, DateTime.Today, 0, false, 1);

                    int orientacion = 1;
                    int movimientoId = new BusPedidos().AgregaMovimientoMaster("MOVIMIENTO ENTRADA", 1, orientacion, cmbAlmacenes.SelectedIndex + 1, 0, Program.UsuarioSeleccionado.Id);
