...
namespace Aires.Pantallas
{
    public partial class ReportesGenerales : FormBase
    {
        ...
        public void CargaCmbClientesFromPedidos(List<EntPedido> PedidosVentasCliente)
        {
            List<EntPedido> lst = new List<EntPedido>();
            foreach (EntPedido p in PedidosVentasCliente)
            {
                lst.Add(new EntPedido() { Cliente= p.Cliente, ClienteId=p.ClienteId});
            }
            lst.Insert(0, new EntPedido() { ClienteId = -1, Cliente= "-TODOS-" });
            cmbClientesVentas.DataSource = lst;
        }
        ...
        public void CargaRvVentas(EntEmpresa EmpresaSeleccionada, DateTime FechaDesde, DateTime FechaHasta)
        {
            ...
            //if (ClienteId > 0)
            //    entPedidoBindingSource.DataSource = this.ListaPedidos.Where(P => P.ClienteId == ClienteId);
            //else
        ...
        {
            entPedidoBindingSource.DataSource = new BusPedidos().ObtienePedidosClientesDeuda(Program.EmpresaSeleccionada.Id);
        ...
        DateTime FechaDesdeVentas
        {
            get
            {
                if (rdoPorMesVentas.Checked)
                    return new DateTime(ConvierteTextoAInteger(cmbAñosVentas.Text), ConvierteTextoAInteger(cmbMesesVentas.SelectedIndex.ToString()) + 1, 1);
                else
                    return dtpFechaDiaVentas.Value.Date;
            }
        }
        DateTime FechaDesdeHasta
        {
            ...
            {
                if (rdoPorMesVentas.Checked)
                    return new DateTime(ConvierteTextoAInteger(cmbAñosVentas.Text), cmbMesesVentas.SelectedIndex + 1, DateTime.DaysInMonth(ConvierteTextoAInteger(cmbAñosVentas.Text), cmbMesesVentas.SelectedIndex + 1));
                else
                    return dtpFechaDiaVentas.Value.Date;
            }
        }
        void FiltraVentas(int ProductoFiltroId, string ProductoFiltroDescripcion, int ClienteFiltroId)
        {

            List<EntPedido> lstFiltro=this.ListaPedidos;

            if (ClienteFiltroId > 0)//1:PENDIENTE; 2:PAGADA
                lstFiltro = lstFiltro.Where(P => P.ClienteId == ClienteFiltroId).ToList();

            if (ProductoFiltroId > 0)//1:PENDIENTE; 2:PAGADA
        ...
        private void btnRefrescarVentas_Click(object sender, EventArgs e)
        {
            try
            {
                if (rdoPorMesVentas.Checked)
                {
                    ...
                    //ObtieneClienteFromCmb(cmbClientesVentas).Id);
                }
                ...
                {
                    //ReporteVentasCargado = false;
                    CargaCmbProductos();
                    CargaCmbClientesFromPedidos(this.ListaPedidos);
                    //ReporteVentasCargado = true;
                    //FiltraVentas(ObtieneProductoFromCmb(cmbFiltroProductosVentas), Convert.ToInt32(cmbClientesVentas.SelectedValue));
                }
            }
            catch (Exception ex) { MuestraExcepcion(ex); }
        }
        ...
        private void cmbClientesVentas_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                //else if (tcReportes.SelectedIndex == 5) //tcReportes.TabPages["Ventas"])
                //{
                //    btnRefrescarVentas.PerformClick();
                //}
            }
        ...
        private void cmbProductosVentas_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                if (tcReportes.SelectedIndex == 1)
                {
                    if (ReporteVentasCargado)
                    {
                        if (this.ListaPedidos != null)
